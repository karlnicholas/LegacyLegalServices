...
<body>
  ...
  <p><xsl:element name="a">
    <xsl:attribute name="href">http://<xsl:value-of select="verifyHost"/>/opinions/views/optout/optout.xhtml?email=<xsl:value-of select="email"/>&quot;key=<xsl:value-of select="optoutKey"/></xsl:attribute>
