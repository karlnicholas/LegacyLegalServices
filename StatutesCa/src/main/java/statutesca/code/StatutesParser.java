...
public class StatutesParser {
   ...
   public StatutesRoot parse(IStatutesApi iStatutesApi, Charset encoding, Path file) throws FileNotFoundException  { 
       ...
       Scanner scanner = new Scanner( file.toFile(), encoding.toString() );
...
//               if ( cline == null && nline == null ) break;
               ...
               if ( cline.length() != 0 ) {
                   if ( cline.trim().charAt(0) != '.' ) {
            		   ...
            		   int depth = secStack.size();
            		   List<StatutesBaseClass> l = parent.getReferences();
            		   String fullFacet = parent.getFullFacet() + "/" + facetHead + "-" + depth + "-" + (l == null ? 0 : l.size());
                	   if ( range == null ) {
...
//                    	   codeMember = new Subcode( parent, part, partNumber, title, (secStack.size()>0?secStack.size():1) );
                    	   codeMember = new StatutesNode( parent, fullFacet, part, partNumber, adjTitle, depth );
                       } else {
...
//                    	   codeMember = new Section(parent, part, partNumber, title, range, (secStack.size()>0?secStack.size():1) );
                    	   codeMember = new StatutesLeaf(parent, fullFacet, part, partNumber, adjTitle, depth, range);
                       }
                       

                	   if ( 
	        			   codeMember.getPart() == null &&
	        			   codeMember.getPartNumber() == null &&
	        			   codeMember.getStatuteRange().getsNumber() == null &&
	        			   codeMember.getStatuteRange().geteNumber() == null
        			   ) { 
                		   System.out.println("Invalid codeMember: " + codeMember);
                	   } else {
...
//           range = new CodeRange(codeSection.substring(0, codeSection.length()) );
           range = new StatuteRange(
        		   new SectionNumber(-1, sNumber), 
        		   new SectionNumber(-1, eNumber) 
        		   );
